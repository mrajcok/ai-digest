...
def _record(company: str, n: int, first_scraped_at: str, category: str = "news") -> ArticleRecord:
...
def test_articles_first_seen_on_filters_by_first_scraped_date(db: ArticleDB) -> None:
    db.upsert(_record("anthropic", 0, f"{TODAY}T10:00:00+00:00"))
    db.upsert(_record("anthropic", 1, f"{YESTERDAY}T10:00:00+00:00"))

    records = db.articles_first_seen_on(TODAY, ["anthropic"])

    assert len(records) == 1
    assert records[0].url.endswith("/0")


def test_articles_first_seen_on_excludes_other_companies(db: ArticleDB) -> None:
    db.upsert(_record("anthropic", 0, f"{TODAY}T10:00:00+00:00"))
...
def test_articles_first_seen_on_respects_limit(db: ArticleDB) -> None:
    for i in range(5):
        db.upsert(_record("anthropic", i, f"{TODAY}T10:00:00+00:00"))

    records = db.articles_first_seen_on(TODAY, ["anthropic"], limit=2)

    assert len(records) == 2


def test_articles_first_seen_on_excludes_missing_summary(db: ArticleDB) -> None:
    record = _record("anthropic", 0, f"{TODAY}T10:00:00+00:00")
    record = record.model_copy(update={"summary": ""})
    db.upsert(record)

    assert db.articles_first_seen_on(TODAY, ["anthropic"]) == []


def test_daily_overview_round_trips(db: ArticleDB) -> None:
    overview = DailyOverview(
        day=TODAY, text="Today...", article_count=3,
...
def test_daily_overview_upsert_overwrites(db: ArticleDB) -> None:
    db.upsert_daily_overview(
        DailyOverview(day=TODAY, text="v1", article_count=1, source_hash="a", model="m", generated_at="t")
    )
    db.upsert_daily_overview(
        DailyOverview(day=TODAY, text="v2", article_count=2, source_hash="b", model="m", generated_at="t2")
    )
...
def test_latest_daily_overview_returns_most_recent(db: ArticleDB) -> None:
    db.upsert_daily_overview(
        DailyOverview(day=YESTERDAY, text="old", article_count=1, source_hash="a", model="m", generated_at="t")
    )
    db.upsert_daily_overview(
        DailyOverview(day=TODAY, text="new", article_count=1, source_hash="b", model="m", generated_at="t")
    )
...
def test_template_renders_todays_overview(db: ArticleDB, tmp_path: Path) -> None:
    ...
    db.upsert_daily_overview(
        DailyOverview(
            day=TODAY, text="Today's big news.", article_count=1,
...
def test_template_falls_back_to_most_recent_overview_labelled(db: ArticleDB, tmp_path: Path) -> None:
    ...
    db.upsert_daily_overview(
        DailyOverview(
            day=YESTERDAY, text="Yesterday's news.", article_count=1,
