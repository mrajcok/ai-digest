...
Configuration mirrors stock-tracker's ``notify.py``: ``EMAIL_ENABLED``,
``SMTP_HOST``, ``SMTP_PORT``, ``SMTP_USER``, ``SMTP_PASSWORD``, ``EMAIL_FROM``,
``EMAIL_TO`` (comma-separated). Values come from the environment first, then
``.autopilot.env`` if you keep autopilot's credentials separate, then ``.env``.

``Settings`` in ``src/digest/config.py`` sets ``extra="ignore"`` so these keys
can share ``.env`` with the pipeline's own settings.
...
def main() -> int:
    ...
    # First file to define a key wins, and the environment beats both.
    load_dotenv(Path(".autopilot.env"))
