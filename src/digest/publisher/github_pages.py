    ...
    def _push_to_github(self, html_files: dict[str, str]) -> None:
        ...
        try:
            ...
            if repo.is_dirty(index=True):
                ...
                repo.index.commit(f"chore: update AI digest [{ts}]")
...
def _group_by_month(records: list[ArticleRecord]) -> list[tuple[str, list[ArticleRecord]]]:
    ...
    def _month_sort(item: tuple[str, list[ArticleRecord]]) -> str:
        label = item[0]
        try:
            # DTZ007 is suppressed below: this re-parses a display label ("July 2026")
            # purely to build a sort key. There is no instant to attach a timezone to.
            return datetime.strptime(label, "%B %Y").strftime("%Y-%m")  # noqa: DTZ007
        except ValueError:
            return ""
