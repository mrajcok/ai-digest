...
can push through the project's own code — this project's publisher pushes to
gh-pages — and the skill permits that where a step calls for it. Treat "did
anything get published?" as something to check in the run log, not assume.

A step is never committed if it modified autopilot.sh, autopilot_notify.py or
the plan-step skill. The skill forbids that, but `git add -A` would commit it
regardless, and bash reads this script incrementally from disk — an in-place
rewrite can make the *running* interpreter execute garbage mid-run.
...
EMAIL_FROM / EMAIL_TO from the environment, then .autopilot.env, then .env —
see .env.example. config.py's Settings sets extra="ignore" so these can share
.env with the pipeline's own settings. With EMAIL_ENABLED unset or false
nothing is sent, and a mail failure never fails the run.
...
# This script itself, as git sees it. Paired with SKILL_FILE and NOTIFY_FILE as
# the paths a step is never allowed to have modified — see the harness guard.
SELF_FILE="autopilot.sh"
NOTIFY_FILE="autopilot_notify.py"
...
send_run_email() {
  ...
  if [[ -n "$step_log" && -f "$step_log" ]]; then
    "./$NOTIFY_FILE" "$subject" < "$body" >> "$step_log" 2>&1
  else
    "./$NOTIFY_FILE" "$subject" < "$body"
  fi
}
...
while (( steps_run < MAX_STEPS )); do
  ...
  harness_edits="$(git status --porcelain -- "$SELF_FILE" "$SKILL_FILE" "$NOTIFY_FILE")"
  if [[ -n "$harness_edits" ]]; then
    stop "Step $step_num modified its own harness. Stopping for review — refusing to commit changes to $SELF_FILE, $SKILL_FILE or $NOTIFY_FILE:"
